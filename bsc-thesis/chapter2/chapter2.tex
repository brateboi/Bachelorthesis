...
Thus, the $3\times3$ 1-forms $\omega_j^k$ define a connection and
...
\begin{figure}[htb]
  \centering
  \def\svgwidth{30em}
  \input{figures/seamlessmap.pdf_tex}
...
and contain types of singularities that cannot appear in hex meshes and are non-meshable~\cite{Liu,locallymeshable}.
...
$$F_u \sim F_v \iff \exists R \in \mathcal{O} : F_u=F_vR$$
...
make $M$ ambiguous (${M_1 = M_2R, R \in \mathcal{O}}$).
...
The restriction makes the rotated polynomials naturally invariant under $\mathcal{O}$.
